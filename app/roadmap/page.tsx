        ...
        {roadmap.map((phase, idx) => (
          <div key={phase.phase} className="relative flex gap-8">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full border-4 border-neutral-950 flex items-center justify-center text-xs font-bold ${phase.status === 'completed' ? 'bg-orange-500 text-black' : phase.status === 'in-progress' ? 'bg-orange-500/20 text-orange-500' : 'bg-neutral-800 text-neutral-500'}`}>
                {idx + 1}
              </div>
            ...
            <div className="flex flex-col gap-4 pb-12">
              ...
              <ul className="flex flex-col gap-2">
                {phase.items.map(item => (
                  <li key={item} className="text-neutral-400 flex items-start gap-2">
