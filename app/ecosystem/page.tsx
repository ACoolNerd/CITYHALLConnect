  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-24 flex flex-col gap-12">
      <header className="flex flex-col gap-4">
        ...
        <h1 className="text-5xl font-black text-white tracking-tighter">ACoolECOSYSTEM Map</h1>
        <p className="text-xl text-neutral-400 leading-relaxed">
          The hub-and-spoke architecture of all interconnected brands, data assets, and governance nodes.
        </p>
      </header>

      <section className="p-12 rounded-3xl bg-neutral-900 border border-neutral-800 text-center">
        <div className="text-neutral-600 font-mono text-sm mb-8">[ VISUAL ECOSYSTEM GRAPH PLACEHOLDER ]</div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {["Holdings (01)", "Nerd (02)", "Trust (03)", "Academy (04)", "Sing (06)", "Finance (07)"].map(node => (
                <div key={node} className="p-4 rounded-xl border border-neutral-800 bg-black/40 text-neutral-400 text-xs font-bold uppercase tracking-widest">
                    {node}
                </div>
            ))}
        </div>
